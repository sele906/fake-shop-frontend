...
접히는 자리를 페이지 단위로 잡았다. 16개 네임스페이스를 다 돌았고, 아래는
...
| promo | 16 | 완료 | 0 |
| common | 16 | 완료 | 0 |
| onboarding | 5 | 완료 | 0 — 제목은 clamp, 아래 `<br />` 절 참고 |

16개 중 13개가 0개로 끝났다. 오버라이드가 붙은 셋(main · careers · detail)은
...
`<br />`로 두 줄을 잡아 둔 문구는 좁은 화면에서 한 줄이 더 접혀 세 줄이 된다.
결제 완료 제목(`done.titleLine1` · `2`)이 그랬다. 온보딩 제목
(`onboarding:title.one` · `two`)도 같은 모양이라 처음부터 이 방법으로 잡았다.

**px를 줄이는 것으로는 못 막는다.** 필요한 폭은 글자 크기에 비례해 줄지만 화면
폭은 안 따라오므로, 어떤 값을 고르든 그보다 좁은 기기에서 다시 뚫린다. 32px을
30px로 낮춰 390px 폰을 통과시켜도 360px에서 또 넘쳤다.

크기를 화면 폭에 비례시켜 비율을 고정한다.
...
계수는 `쓸 수 있는 폭 ÷ 가장 긴 줄의 글자 폭`에서 나온다. 결제 완료는
`.doneIn`이 `max-width: 520px` + 좌우 16px이라 쓸 수 있는 폭이 `화면 − 32px`이고,
한국어 `"아무것도 사지 않았습니다"`가 약 11.4자 폭이라 417px보다 좁아지면 함께
줄어든다. `balance`는 안전망이다 — 그래도 넘치면 두 줄을 고르게 나눠 준다.

온보딩은 같은 계산에서 `clamp(20px, 6.4vw, 28px)`이 나왔다 — 좌우 20px에
한국어 `"충동구매는 하고 싶지 않은데,"`가 약 13.8자 폭이라 계수가 더 작다.
줄이 길수록 계수가 작아진다.

**본문에는 이 방법을 쓰지 않는다.** 14px 아래로 내려가면 읽기가 나빠진다.
본문이 넘치면 문구를 줄이는 쪽이 맞다 — 위 `순서는 문구 먼저, CSS 나중` 참고.
...
## 다크 모드 릴리스 (2026-08-19)

플레이에 올라가 심사를 통과했다. 테스터 14명이 겪던 6건이 함께 풀렸다.

심사 봇이 **로그인 화면을 실제 로그인으로 보고** 한 번 걸었다. 앱 콘텐츠의
`Sign in details`(예전 이름 App access)에 테스트 계정과 설명을 적어 통과했지만,
우리 로그인은 빈칸으로도 통과하므로 사실에 맞는 선언은
`All functionality is available without any access restrictions` 쪽이다.
그렇게 바꿔 두면 다음 업로드부터 안 물어본다.

심사 중에는 선언을 바꾸지 않는다 — 검토가 처음부터 다시 갈 수 있다.
...
고를 수 있는 것은 셋(`light` · `dark` · `system`)이고 실제로 걸리는 것은 둘이다.

**네이티브에는 푼 결과가 아니라 고른 모드를 보낸다.** `"system"`을 우리가 풀어서
`"dark"`로 보내면 안드로이드가 `MODE_NIGHT_FOLLOW_SYSTEM` 대신 고정 모드를 쓰게
되어 OS가 바뀌어도 안 따라간다. 반대로 `theme-color`와 상태바 아이콘 색은 지금
걸린 배경을 봐야 하므로 푼 결과를 쓴다.

**OS 변화 구독이 따로 필요하다.** 부팅 스크립트는 뜰 때 한 번 읽을 뿐이라 앱을
열어 둔 채 OS가 바뀌면 못 따라간다. 시스템 테마를 별도 state로 둔 것은
`"system"`인 채로 OS만 바뀌면 `mode` 값이 그대로라 리렌더가 안 걸리기 때문이다.

저장값이 `"dark"`도 `"light"`도 아니면 OS를 따른다. **고른 적 없는 상태와**
**`"system"`을 같게 다루는 것이 의도다** — 처음 여는 사람과 시스템으로 되돌린
사람이 같은 화면을 본다.
