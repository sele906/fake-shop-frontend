...
## 다음 할 일 — 다크 모드까지 묶어서 릴리스

### 지금 어디까지 왔나 (2026-08-18)

테스터 14명에게 받은 피드백 대응은 커밋·푸시까지 끝났다. `app-release.aab`도
만들어 뒀다 — `android/app/release/`, 17.1 MB. 영어 앱 이름이 들어갔는지는
`base/resources.pb`를 꺼내 확인했다(`app_name` 기본 `안삼` + `en` 조건 `NOBUY`).

**다만 플레이에 아직 올리지 않았다.** 그래서 이 AAB는 버려도 된다. 아래를
끝내고 다시 만들어 한 번에 올린다.
...
**올리는 것을 다크 모드까지 미루기로 했다.** 앱 바이너리는 올릴 때마다 구글
심사를 다시 타므로 한 번으로 묶는 쪽이 싸다.

대신 대가가 있다 — 지금 빌드에 이미 고쳐진 6건을 테스터 14명이 그동안 계속
겪는다. **다크 모드가 며칠 넘게 길어지면 이 판단을 다시 본다.** 지금 것을 먼저
올리고 다크 모드를 다음 릴리스로 미루는 편이 나아지는 지점이 있다.

### 먼저 — 상태바는 다크 모드와 별개다

테스터가 **검은 상태바 위에 흰 헤더**가 붙은 스크린샷을 보냈다. 다크 모드와
무관하게 지금도 어긋나 있다. 액티비티 테마(`AppTheme.NoActionBar`)가
`statusBarColor`를 잡지 않아 시스템 기본값이 나온다.

```xml
<!-- values/styles.xml의 AppTheme.NoActionBar에 -->
<item name="android:statusBarColor">#f4f5f7</item>    <!-- --bg와 같은 값 -->
<item name="android:windowLightStatusBar">true</item>  <!-- 밝은 배경 → 어두운 아이콘 -->
```

**플러그인은 필요 없다.** `@capacitor/status-bar`는 화면마다 색을 바꿀 때 쓰는
것이고, 여기는 색이 하나다. 다크 모드까지 가면 `values-night/styles.xml`에
어두운 짝을 둔다.
...
| | |
| --- | --- |
| `index.css` `:root`의 색 토큰 | 27개 |
| CSS 모듈의 `var(--...)` 사용 | 764곳 |
| 하드코딩된 색 | 146곳 |

**색의 대부분이 이미 토큰을 지난다.** 그래서 뼈대는
`@media (prefers-color-scheme: dark)` 안에서 `:root` 토큰 27개를 다시 정의하는
것이고, 그것만으로 764곳이 따라온다.

### 진짜 일은 `#fff` 119개다

하드코딩 146개 중 **119개가 `#fff`** 다. 나머지 27개는 흩어진 스크림·그림자라
양이 적다.

```
Coupon 16 · Delivery 14 · Checkout 14 · Login 11
Main 9 · Detail 9 · Cart 9 · Company 6 …
```

**일괄 치환하면 안 된다.** 두 종류가 섞여 있다.

- **테마를 따라야 하는 흰색** — 카드 · 패널 배경. 다크에서는 어두워져야 한다
- **항상 흰색이어야 하는 흰색** — 히어로 사진 위 글자, 강조색 버튼 위 글자.
  배경이 어차피 어두워서 다크에서도 흰색이 맞다

앞의 것만 토큰(`--surface` 등)으로 바꾸고 뒤의 것은 그대로 둔다. 파일을 열어
**그 흰색이 무엇 위에 얹히는지** 봐야 갈린다 — 기계적 치환이 안 되는 유일한
부분이고 여기가 이 작업의 대부분이다.

### 이미 되어 있는 것

- 안드로이드 테마가 `Theme.AppCompat.DayNight.NoActionBar`다. 이미 다크를 안다
- 스플래시에 `night` 밀도 폴더가 13개 있다. 어두운 짝을 넣을 자리가 이미 있다

### 잊기 쉬운 것

- `public/index.html`의 `theme-color`가 값 하나다. 다크용을 하나 더 둔다
  (`<meta name="theme-color" media="(prefers-color-scheme: dark)" ...>`)
- 상품 이미지는 Pexels 사진이라 어두워지지 않는다. 카드 배경만 어두워지면
  사진이 도드라져 뜬다. 테두리 · 그림자를 다시 봐야 할 수 있다
- `AppToaster`(sonner)는 자기 테마를 들고 있다. 페이지와 따로 맞춰야 한다
