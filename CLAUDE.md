...
## 통화 — 원으로 계산하고 그릴 때만 바꾼다

영어에서는 달러로 나온다(`37,800원` → `$27.00`). 테스터 요청으로 붙였다.
기호만 `$`로 바꾸는 안은 버렸다 — `$37,800`은 과자 한 봉지가 5천만 원으로 읽힌다.

**환율은 `lib/usePrice`의 `WON_PER_UNIT` 한 곳뿐이다.** `USD: 1400` 고정값이다.
실시간으로 받지 않는 이유는 셋이다 — 밖으로 나가는 연결이 늘면 개인정보
처리방침을 고쳐야 하고, 오프라인에서 금액이 사라지고, 같은 영수증 링크의 금액이
날마다 달라진다.

| 정하는 것 | 자리 |
| --- | --- |
| 어느 통화인가 | `common.json`의 `currency` (`KRW` · `USD`) |
| 단위를 붙이는 모양 | `common.json`의 `price` (`{{amount}}원` · `${{amount}}`) |
| 자릿수 구분 | `common.json`의 `intlLocale` |
| 소수 자릿수 | 적지 않는다. `Intl`이 통화마다 안다 (KRW 0 · USD 2) |

`currency`는 두 언어에 다 있어야 한다. 없으면 `t()`가 키 이름을 돌려주고
`Intl`이 잘못된 통화 코드로 예외를 던진다.

**합산 · 할인은 원 정수로 끝낸 뒤 한 번만 바꾼다.** 줄마다 달러로 바꿔 더하면
반올림이 쌓여 상품 합과 총액이 1센트씩 어긋난다. 상품 가격 · 무료배송 기준
(`FREE_SHIP`) · 배송비 · 영수증 링크의 `t`가 전부 원이다.

단위 없이 숫자만 필요하면 `price.amount(won)`을 쓴다. 영수증 배지
`잔고 방어 +{{amount}}`가 한국어에서도 단위를 안 붙이고 있어 이쪽을 쓴다.
**`toLocaleString`을 직접 부르면 환율을 건너뛴다.** 배지가 그렇게 원화 숫자를
찍고 있었다.

### 쿠폰 문구는 원화 원본으로 둔다

`benefit`은 `"5,000원"` · `"₩5,000"` · `"7%"` 같은 문구이고, `readBenefit`이
숫자만 뽑아 **원으로 읽는다.** 그래서 `data/en/coupon.json`에 `"₩5,000"`이
남아 있는 것은 의도다.

- `"$3.57"`로 고치면 `readBenefit`이 357원으로 읽는다
- 받은 쿠폰은 받을 때의 문구째 localStorage에 저장된다. 원본을 고쳐도 이미 받은
  쿠폰은 옛 문구다

화면에는 `benefit`을 그대로 그리지 말고 `coupon/useBenefit`을 거친다. 비율은
그대로, 금액은 `usePrice`로 다시 그린다. 지금 5곳이다(쿠폰함 3 · 장바구니 1 ·
숨은 쿠폰 토스트 1).

### 통화가 아닌 것 · 문구에 박힌 0

- **적립 포인트 `P`는 통화가 아니다.** 원 금액의 5%를 그대로 쓴다. 영어에서는
  `$27.00` 상품에 `1,890P`가 붙는다. 알고 둔 것이다
- 문구에 박힌 0은 자리에 따라 갈랐다. 합계 칸(`cart` · `checkout`의 `sum.zero`)은
  위아래 금액과 줄을 맞춰 `$0.00`, 버튼 · 통계 · 도움말은 문장 안이라 `$0`

### 검증할 때 — 공유 링크는 운영으로 열린다

`lib/shareUrl`이 운영 주소를 박아 두어서, 로컬에서 복사한 `/receipt?d=...` 링크도
Vercel을 연다. 배포 전에 그 링크로 확인하면 **옛 코드를 보게 된다.** 실제로
"영어 브라우저인데 원화"를 언어 저장값 문제로 잘못 짚은 적이 있다. 도메인을 로컬
주소로 바꿔서 연다.
...
| | 실제 상태 |
| --- | --- |
| 스플래시 | `AppTheme.NoActionBarLaunch`(`Theme.SplashScreen`)와 `MainActivity`의 `installSplashScreen`이 띄운다 — 아래 참고 |
| 상태바 | 색만 바꾸는 것은 `styles.xml`에 `android:statusBarColor` 한 줄이다. 플러그인은 화면마다 색을 바꿀 때만 필요하다 |
| 뒤로가기 | 아무도 처리하지 않고 있었다 — 아래 참고 |

### 스플래시는 웹이 뜰 때까지 붙잡았다가 걷는다

테마만 걸려 있고 `installSplashScreen` 호출이 없던 때는, 로고가 액티비티 첫
프레임에서 뚝 끊기고 아직 빈 웹뷰가 한 번 비쳤다. "애니메이션이 부족하다"는
테스터 피드백에서 고쳤다.

`MainActivity`가 하는 일은 셋이다.

1. `installSplashScreen` — `super.onCreate` 앞. 테마를 launch에서
   `postSplashScreenTheme`로 넘기는 일을 여기서 한다
2. `setKeepOnScreenCondition` — 웹뷰의 `onPageLoaded`(또는 `onReceivedError`)까지
   붙잡는다. 페이지가 끝내 안 떠도 3초 뒤에는 걷는다
3. `setOnExitAnimationListener` — 400ms 페이드. 콜드 스타트에서만 불린다

JS 신호를 따로 두지 않고 페이지 로드를 기다린다. 걷힌 뒤 데이터 로딩 화면이
잠깐 보일 수 있는데, 그건 웹이 그리는 의도한 화면이라 빈 판과 다르다.
같은 이유로 `@capacitor/splash-screen`은 쓰지 않는다 — 숨는 시점을 JS에서
정하는 플러그인인데, 네이티브 20줄로 끝난다.

| 리소스 | 맡는 일 |
| --- | --- |
| `res/values{,-night}/colors.xml`의 `splashBg` | 바탕. 라이트 흰색 · 다크 `#1b1e23` |
| `res/drawable/splash_icon.xml` | 로고를 16.7% 들여 넣는 inset |
| `res/mipmap-*/splash_logo.png` | 흰 배경을 투명으로 바꾼 쇼핑백 |

**스플래시는 앱 토글이 아니라 OS 다크 설정을 따른다.** 앱 코드가 돌기 전에
시스템이 그리는 창이라 `ThemePlugin`의 나이트 모드가 닿지 않는다. 앱은 다크,
OS는 라이트인 사람은 흰 로고 화면에서 어두운 앱으로 페이드된다.

**런처 전경(`ic_launcher_foreground`)을 로고로 쓰면 안 된다.** 그 PNG는 배경이
불투명한 흰색이라, 어두운 바탕에서 흰 원이 오려져 드러난다. `splash_logo`는 같은
그림에서 흰색만 알파로 되돌린 것이라 **런처 아이콘을 다시 만들면 함께 다시
뽑아야 한다.**

**16.7%를 들이는 이유** — 스플래시는 아이콘 칸의 가운데 2/3만 원으로 오려
보여 준다. 그대로 넣으면 쇼핑백 아래 모서리가 원 밖으로 나가 잘린다. 적응형
아이콘과 같은 값이다.

`styles.xml`에서 옛 `android:background="@drawable/splash"`를 뺐다. 이 속성은
창이 아니라 모든 뷰에 걸려서, 안드로이드 11 이하에서는 호환 라이브러리가 그리는
로고 위에 옛 이미지가 한 벌 더 깔린다. `drawable-*/splash.png` 26개 폴더는 이제
아무도 참조하지 않지만 지우지 않았다.

**확인은 런처 아이콘으로 한다.** Android Studio에서 곧바로 띄우면 스플래시
아이콘이 안 나오는 경우가 있다. 앱을 완전히 끄고 아이콘을 눌러 연다.
...
### `inset`을 쓰지 않는다 — 오래된 웹뷰

앱은 기기의 시스템 웹뷰로 그려진다. 최신 브라우저에서 확인한 CSS가 그대로
나온다는 보장이 없다. 안드로이드 11 에뮬레이터의 기본 웹뷰가 Chromium 83이었고,
`minSdk`가 24(안드로이드 7)라 업데이트 안 된 기기는 더 낡았을 수 있다.

**`inset: 0`은 Chromium 87부터다.** 모르는 웹뷰에서는 선언이 통째로 버려져
위치값 없는 `position: fixed`가 되고, 요소가 문서 흐름상 제자리에 놓인다.

온보딩이 이걸 밟았다. `App.js`에서 페이지 뒤에 그려지므로, 데이터가 들어와
메인 페이지가 길어지는 순간 페이지 맨 아래로 밀려났다. 사용자에게는 **안내가
뜨자마자 저절로 사라진 것**으로 보였다 — 닫힘 로직을 의심하게 만드는 모양이라
찾기 어렵다.

같은 선언이 9곳 있었고 전부 네 줄로 풀었다. 최신 브라우저에서는 결과가 같다.
...
CRA의 PostCSS는 `inset`을 풀어 주지 않는다(설정도 못 바꾼다). 새로 적을 때
손으로 풀어야 한다.

83에서 안 되는 것은 더 있다 — flex `gap`(84+), `aspect-ratio`(88+, 7개 파일
12곳). 이것들은 **아직 83에서 화면별로 확인하지 않았다.** 메인 목록은 사진이
로드된 뒤 정상으로 보였지만, 로드 전 프레임에서 사진 칸이 비어 있었다.
**"뜨자마자 사라진다" · "자리를 잃는다"면 먼저 웹뷰 버전을 본다.**
